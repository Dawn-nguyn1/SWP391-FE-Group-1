...
const resendRegisterOTPAPI = (email) => {
    ...
    const URL_BACKEND = `/api/auth/register/resend-otp?email=${encodeURIComponent(email)}`;
...
const getApprovedOrdersAPI = (page = 0, size = 20, sort = ['createdAt,desc', 'id,desc']) =>

    axios.get('/api/operation_staff/orders/approved', { params: { page, size, sort } });
...
export {
    ...
    activatePreorderCampaignAPI, deactivatePreorderCampaignAPI
